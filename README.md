...
These elements are described in different folders. All the modelling happens within the `openfisca_celestia` folder.
...
```sh
pip install openfisca_celestia
```
...
```sh
git clone https://github.com/openfisca/openfisca-celestia.git
cd openfisca-celestia
...
Substitute your package's country name for `openfisca_celestia` below:

```sh
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_celestia/situation_examples/couple.json \
